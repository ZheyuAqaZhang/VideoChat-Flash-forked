...
class Qwen2Model(Qwen2PreTrainedModel):
    ...
    def forward(
        ...
        for layer_idx, decoder_layer in enumerate(self.layers):
            ...
            ###### copy from VideoChat-Flash #########
            # rank & drop after specific layer
            # only drop in prefill stage when inference
            rank_layer = layer_idx+1
            llm_compress_layer_list = eval(os.environ.get("EXTRA_PARAM_INNER_CONDENSER_ID", "[]"))
            if rank_layer in self.llm_compress_layer_list:
                if hidden_states.shape[1] != 1:  # prefill stage or training
                    ...
                    stage = self.llm_compress_layer_list.index(rank_layer) # determine current stage
                    cur_visual_length = [int(cur_image_token * self.llm_image_token_ratio_list[stage] + 1e-6) for cur_image_token in self.num_image_token_lens]
                    next_visual_length = [int(cur_image_token * self.llm_image_token_ratio_list[stage + 1] + 1e-6) for cur_image_token in self.num_image_token_lens]
                    new_position_ids = []
    ...
    def flash_rank_drop(
        ...
        position_ids, attention_mask, labels, condenser
    ):
        ...
        if getattr(self.config, 'tokenizer_padding_side', 'right') == "right":
            ...
            image_tokens = [int(cur_image_token * self.llm_image_token_ratio_list[cur_num] + 1e-6) for cur_image_token in self.num_image_token_lens]
            keep_length = [int(cur_image_token * self.llm_image_token_ratio_list[cur_num + 1] + 1e-6) for cur_image_token in self.num_image_token_lens]
            ...
            for i in range(batch_size):
                ...
                old_image_embeds = features[i][image_index: image_index + image_tokens[i]]
                
                # import pdb; pdb.set_trace()
                inner_condenser = condenser.inner[cur_num]
                new_image_embeds = inner_condenser(old_image_embeds.unsqueeze(0)).squeeze(0)
                # original version: new_image_embeds = features[i][ top_rank_index, :]
                assert new_image_embeds.shape[0] == keep_length[i], "new_image_embeds shape is not correct"

                start_index = image_index + image_tokens[i]
                new_input_embeds = torch.cat([features[i][ :image_index, :] , new_image_embeds, features[i][start_index:, :]], dim=0)
